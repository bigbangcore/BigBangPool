...
修改package.json的如下两项（先去下载两个库，存放到你本地目录，然后……）：  
"cryptoforknote-util": "git+file:///home/chasibu/BigBang/node-cryptoforknote-util",  
...
/* 用于币种存储标识，存入redis里面，这样多个币种可以共用同一个redis实例*/
"coin": "bigbang",
...
"poolServer": {
    ...
    /* Variable difficulty is a feature that will automatically adjust difficulty for
       ...
       overhead. 
       可变难度是一个功能，它将根据散列率自动调整单个矿工的难度，以降低网络和CPU开销*/
    "varDiff": {
        "minDiff": 8, // Minimum difficulty,最小难度
        "maxDiff": 200,//最大难度
        "targetTime": 60, // Try to get 1 share per this many seconds，尝试在这么多秒内获得1个共享
        "retargetTime": 30, // Check to see if we should retarget every this many seconds，每隔几秒钟重新定位一次
        "variancePercent": 15, // Allow time to vary this % from target without retargeting，允许时间在不重定向的情况下从目标中改变此百分比
        "maxJump": 100 // Limit diff percent increase/decrease in a single retargeting，难度调整率
    },
    ...
    如果在低难度共享攻击下，我们可以禁止他们的IP以减少系统/网络负载。*/
    "banning": {
        "enabled": true,
        "time": 600, // How many seconds to ban worker for，禁止工作时长
        "invalidPercent": 25, // What percent of invalid shares triggers ban，引发禁令的无效票百分比
        "checkThreshold": 30 // Perform check when this many shares have been submitted，执行检查
    },
    ...
    Slush Mining是一种奖励计算技术，它可以抑制跳槽行为，并通过将较年轻的股票估值高于较年长的股票来奖励“忠诚”的矿工。记住调整权重！
...
"payments": {
    ...
    "maxAddresses": 50, // Split up payments if sending to more than this many addresses，如果发送到超过此多个地址，则拆分付款
    "mixin": 5, // Number of transactions yours is indistinguishable from
    "priority": 0, // The transaction priority,交易优先权
    "transferFee": 1, // Fee to pay for each transaction，每一笔交易的交易费用
    "dynamicTransferFee": true, // Enable dynamic transfer fee (fee is multiplied by number of miners),启用动态交易费（费用乘以矿工数）
    // 使用动态转移费时，矿工支付转移费而不是池所有者
    "minerPayFee" : true, // Miner pays the transfer fee instead of pool owner when using dynamic transfer fee
    "minPayment": 600000, // Miner balance required before sending payment，支付矿工费用时矿工账户余额
    "maxPayment": null, // Maximum miner balance allowed in miner settings，最大余额
    "maxTransactionAmount": 0, // Split transactions by this amount (to prevent "too big transaction" error)，按此金额拆分交易（以防止“太大交易”错误）
    "denomination": 100000 // Truncate to this precision and store remainder，截断到这个精度并存储余数
},
...
"blockUnlocker": {
    ...
    // 一些分叉币在rpc请求中存在块高问题，要修复此问题，可以启用此选项。
...
"monitoring": {
    "daemon": {
        "checkInterval": 60, // Interval of sending rpcMethod request,发送rpcmethod请求的间隔
