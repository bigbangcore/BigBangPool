    ...
    async.waterfall([
        ...
        function (balances, minPayoutLevel, callback) {
            ...
            let unlock_rpcRequest = {
                pubkey:config.poolServer.spentAddress,
            ...
            apiInterfaces.rpcWallet("unlockkey",unlock_rpcRequest,function(error,pubKey){
                ...
                async.filter(transferCommands, function (transferCmd, cback) {
                    ...
                    apiInterfaces.rpcWallet("sendfrom", rpcRequest, function (error, result) {
                        if (error) {
                            log('info', logSystem, 'Error with %j RPC request to wallet daemon %j', [rpcRequest, error]);
                    ...
                    for (let m in notify_miners) {
                        ...
                        log('info', logSystem, 'Payment of %s to %s', [tmpAmount, notify.address]);
                    ...
                    if (parseInt(totalAmount) > 10000) {
                        ...
                        redisClient.zadd(config.coin + ':message:payment', now, [
                            now,
                            transferCommands.length,
                            succeeded.length,
                            failedAmount,
                            utils.toFixed(totalAmount)
