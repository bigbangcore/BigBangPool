...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        ...
        case 'getjob':
            if (!miner) {
                sendReply('Unauthenticated_getjob');
                return;
            }
            miner.heartbeat();
        ...
        case 'submit':
            if (!miner) {
                sendReply('Unauthenticated_submit');
                return;
            }
            miner.heartbeat();
        ...
        case 'keepalived':
            if (!miner) {
                sendReply('Unauthenticated_keepalived');
                return;
            }
            miner.heartbeat();
            if (IsBannedIp(ip +':' + miner.workerName)) {
...
function newConnectedWorker(miner) {
    log('info', logSystem, 'Miner connected %s@%s on port: %s, name: %s', [miner.login, miner.ip, miner.port, miner.workerName || 'undefined']);
    ...
    if (!(miner.ip === 'undefined' || miner.ip === '')) {
