    ...
    async.waterfall([
        ...
        function (callback) {
            redisClient.zrange(config.coin + ':blocks:candidates', 0, -1, 'WITHSCORES', function (error, results) {
                if (error) {
                    log('error', logSystem, 'Error trying to get pending blocks:%j', [error]);
                    callback(true);
                    return;
                }
                if (results.length === 0) {
                    log('info', logSystem, 'No blocks candidates in database');
                    callback(true);
                    return;
                }
                else {
                    log('info', logSystem, '%d blocks candidates in database', [results.length]);
                }
        ...
        function (blocks, callback) {
            ...
            let currentHeight = 0;
            apiInterfaces.rpcDaemon("getforkheight",{},function(error,res){
                if(error){
                    log('error',logSystem,"Error whith getforkheight:%j",[error]);
                    callback(true);
                    return;
                }
                currentHeight = res;
        ...
        function (unlockedBlocks, callback) {            
            ...
            redisClient.multi(redisCommands).exec(function (error, replies) {
                if (error) {
                    log('error', logSystem, 'Error with getting round shares:%j', [error]);
                    callback(true);
                    return;
                }
        ...
        function (unlockedBlocks, callback) {
            ...
            unlockedBlocks.forEach(function (block) {                
                if (!block.orphaned) return;//it is not orphaned,return                
                ...
                if (block.workerScores && !slushMiningEnabled) {
                    ...
                    Object.keys(workerScores).forEach(function (worker) {
                        orphanCommands.push(['HINCRBYFLOAT', config.coin + ':scores:roundCurrent', worker, workerScores[worker]]);
                    });
                }
            ...
            if (orphanCommands.length > 0) {
                redisClient.multi(orphanCommands).exec(function (error, replies) {
                    if (error) {
                        log('error', logSystem, 'Error with cleaning up data in database for orphan block(s) %j', [error]);
                        callback(true);
                        return;
                    }
                    callback(null, unlockedBlocks);
        ...
        function (unlockedBlocks, callback) {
            ...
            unlockedBlocks.forEach(function (block) {
                if (block.orphaned) return; // it is orphaned,return
                totalBlocksUnlocked++;
                ...
                let reward = utils.toFixed(block.reward - (block.reward * feePercent), 2);
                let fee = utils.toFixed(block.reward * feePercent , 2);

                payments[wallet] = Number(payments[wallet] || 0) + Number(fee);

                //TODO:USE MySQL to DO IT
                //unlockedBlocksCommands.push(['LPUSH',config.coin + ':payRecord:' + wallet, fee + ':' + block.height + ':' + block.difficulty + ':' + feePercent + ':' + block.time]);
                ...
                if (block.workerScores) {
                    //let totalScore = parseInt(block.score);
                    let totalScore = 0;

                    Object.keys(block.workerScores).forEach(function(worker){
                        totalScore += parseInt(block.workerScores[worker]);
                    });

                    //log('info', logSystem, 'totalScore:%j', [totalScore]);
                    //log('info', logSystem, 'block.workerScores:%j', [block.workerScores]);

                    log('info', logSystem, 'totalScore: %j, block.score: %j', [totalScore, block.score]);
                    if (totalScore > 0){
                        Object.keys(block.workerScores).forEach(function (worker) {
                            //TODO:在这里按照算力分币，检查是否正确
                            let percent = utils.toFixed(block.workerScores[worker] / totalScore);
                            if (percent >=1) {
                                percent = 1;
                            }
                            ...
                            log('info', logSystem, 'worker:%j,workerReward:%j,diff:%j,workerScores:%j,height:%d', [worker, workerReward, block.difficulty, block.workerScores[worker],block.height]);
                            payments[worker] = Number(payments[worker] || 0) + Number(workerReward);
                            
                            //TODO:USE MySQL to DO IT
                            unlockedBlocksCommands.push(['LPUSH',config.coin + ':payRecord:' + worker, workerReward + ':' + block.height + ':' + block.difficulty + ':' + percent + ':' + block.time]);
                            log('info', logSystem, 'Block %d payment to %s for %d%%, total: %j', [block.height, worker, percent * 100, payments[worker] || 0]);
                        });
                    }                    
                }
            ...
            for (let worker in payments) {
                let amount = parseFloat(payments[worker]);
                if (amount <= 0) {
