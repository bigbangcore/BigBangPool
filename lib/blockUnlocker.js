            ...
            unlockedBlocks.forEach(function (block) {
                ...
                if (block.workerScores) {
                    ...
                    Object.keys(block.workerScores).forEach(function (worker) {
                        ...
                        log('info', logSystem, 'worker:%j,workerReward:%j,diff:%j,workerScores:%j', [worker, workerReward, block.difficulty, block.workerScores[worker]]);
                        payments[worker] = Number(payments[worker] | 0) + Number(workerReward);
                        
                        //TODO:insert the data into redis,as a list,for statistics
