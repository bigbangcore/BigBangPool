...
function handleServerRequest(request, response) {
    ...
    switch (urlParts.pathname) {
        ...
        case '/get_allow_lst':
            ...
            handleGetAllowLst(urlParts, response);
            break;
        case '/set_allow_miner':
            if (!authorize(request, response)) {
                return;
            }
            handleSetAllowMiner(request, response);
            break;
        // Pool statistics
        case '/stats':
        ...
        // PayRecords
        case '/get_payRecords':
            handleGetPayRecords(urlParts, response);
            break;

        // PayRecords
        case '/get_message':
            handleGetMsg(urlParts, response);
            break;
...
function collectStats() {
    ...
    let redisCommands = [
        ['zremrangebyscore', config.coin + ':hashrate', '-inf', ''],//移除指定区间的成员
    ...
    redisCommands[0][3] = '(' + windowTime; // 给BBC:hashrate赋值
    ...
    async.parallel({
        pool: function (callback) {
            redisClient.multi(redisCommands).exec(function (error, replies) {
                ...
                /**
                 * 矿池算力  计算的是连接矿池的所有机器600秒内的hashrate总和除以600
                 *  */

                //TODO:检查hashrate
                data.hashrate = Math.round(totalShares / config.api.hashrateWindow);
                data.roundScore = 0;
                highestHashrate = parseInt(data.hashrate) > parseInt(highestHashrate) ? parseInt(data.hashrate) : parseInt(highestHashrate);

                let tmpHashrate = highestHashrate;
                if (parseInt(data.hashrate) < (parseInt(highestHashrate) / 2)){
                    highestHashrate = parseInt(data.hashrate);

                    let now = Date.now() / 1000 | 0;
                    //TODO:USE MySQL to DO IT
                    redisClient.zadd(config.coin + ':message:hashrate', now, [
                        now,
                        tmpHashrate,
                        data.hashrate
                    ].join(':'), function (err, result) {
                        if (err) {
                            log('error', logSystem, 'Failed inserting hashrate message: %j', [err]);
                        }
                    });
                }
                ...
                //log('error', logSystem, 'api-data:%j', [data]);
                callback(null, data);
            });
        },
        lastblock: function (callback) {
            getLastBlockData(function (error, data) {
                ...
                if ((daemonFinished / 1000 - data.timestamp) > 6000 && data.difficulty > 0) {
                    data.hashrate = 0;
                }
                callback(error, data);
            });
        ...
        network: function (callback) {            
            getNetworkData(function (error, data) {
                ...
                //redisClient.del(config.coin + ':networkJson');//清除redis数据
                if ((daemonFinished / 1000 - data.timestamp) > 6000 && data.difficulty > 0) {
                    data.netHashrate = 0;
                }
                callback(error, data);
        ...
        config: function (callback) {
            callback(null, {
                ...
                fee: config.blockUnlocker.poolFee,//0.8
                devDonation: config.blockUnlocker.devDonation,//0.2
                ...
                priceCurrency: config.prices ? config.prices.currency : 'BBC',
        ...
        if (JSON.stringify(results).length > 10) {
            currentStats = results;
            broadcastLiveStats();
        }
        setTimeout(collectStats, config.api.updateInterval * 1000);//5秒钟跑一次
    });
}

function handleGetAllowLst(urlParts, response) {
    let poolNames = ['bbc01', 'bbc02'];//TODD: Get From DB
    let poolName = urlParts.query.poolName || 0;

    if (poolNames.includes(poolName)) {
        let workers = config.coin + ':workers:Allow';
        redisClient.lrange(workers, 0, -1, function(err, result) {
            let data;
            let total = 0;
            if (err) {
...
function handleSetAllowMiner(request, response) {
    let worker = config.coin + ':workers:Allow';
    request.on('data', function (d) {
        ...
        tmpJson += '}';
        redisClient.LPUSH(worker, tmpJson, function(err, result) {
            if (err) {
                log('info',logSystem, 'Set workers allow err:%j',[err]);
                return;
            }
            let reply = '{"result":"SUCCESSFUL"}';
            response.writeHead("200", {
                'Access-Control-Allow-Origin': '*',
                'Cache-Control': 'no-cache',
                'Content-Type': 'application/json',
...
function getNetworkData(callback) {
    async.waterfall([
        ...
        function (data, callback) {
            ...
            if (height !== networkLastHeight) {               
                let tmpHeightCount = height > 30 ? 30 : height;//30             
                for (let i = height; i > height - tmpHeightCount; i--) {                
                    apiInterfaces.rpcDaemon('getblockhash', { height: i }, function (error, hash) {
                        if (error) {
                            log('error', logSystem, 'Error getting block hash! %j', [error]);
                            return callback(false);
                        }
    
                        apiInterfaces.rpcDaemon('getblockdetail', { block: hash.toString() }, function (error, res) {
                            if (error) {
                                log('error', logSystem, 'Error getting last block data %j', [error]);
                                return callback(false);
                            }
                            networkArr.push(res.time + ':' + res.bits + ':' + res.height);
                        });
                    });
                }
            }
            else{
                networkArr = networkArrBac;
            }            
            callback(null, data);
        },
        function (data, callback) {
            ...
            let nTotalTime = 0;
            let nTimeFirst = 0;
            let nTimeLast = data.timestamp;
            for (let i = 0; i < networkArr.length; i++) {
                let nlist = networkArr[i].split(':');
                if (nlist[0] && nlist[1]) {
                    netHashTotal += (Math.pow(2, Number(nlist[1])));
                    if(i === networkArr.length - 1){
                        nTimeFirst = nlist[0];
                    }                    
                }
            }

            nTotalTime = (Number(nTimeLast) - Number(nTimeFirst));            
            let netHashrate = utils.toFixed(netHashTotal / nTotalTime, 2);            
            networkLastHeight = data.height;
            networkArrBac = networkArr;
            networkArr = [];
...
function getLastBlockData(callback) {
    apiInterfaces.rpcDaemon('getforkheight', {}, function (error, hgt) {
        ...
        apiInterfaces.rpcDaemon('getblockhash', { height: hgt }, function (error, hash) {
            ...
            apiInterfaces.rpcDaemon('getblockdetail', { block: hash.toString() }, function (error, result) {
                if (error) {
                    log('error', logSystem, 'Error getting last block data %j', [error]);
                    return callback(true);
                }
                callback(null, {
                    ...
                    txmint: result.txmint,
                    prev: result.prev,
                    difficulty: result.bits,
                    reward: result.txmint.amount
                });
            });
        });
    });
...
function broadcastWorkerStats(address, destinations) {
    ...
    async.waterfall([
        function (callback) {
            redisClient.keys(config.coin + ':payments:*', callback);
        },
        function (keyLst) {
            let addressPay = "";
            for(let i = 0;i < keyLst.length; i++){
                if(keyLst[i].indexOf(address) > 0){
                    addressPay = keyLst[i];
                    break;
                }
            }
            let redisCommands = [
                ...
                ['zrevrange', addressPay, 0, config.api.payments - 1, 'WITHSCORES'],
            ...
            function (keyLst) {
                ...
                redisClient.multi([
                    ...
                    charts.getUserChartsData(address, paymentsData, function (error, chartsData) {
                        let redisCommands = [];
                        for (let i in workersData) {
                            //log('warn', logSystem, 'workersData[%d].name:%j', [i, workersData[i].name]);
                            redisCommands.push(['hgetall', config.coin + ':unique_workers:' + address + '~' + workersData[i].name]);
                            redisCommands.push(['get', config.coin + ':charts:worker_hashrate:' + address + '~' + workersData[i].name]);
                        }
...
function handleGetPayments(urlParts, response) {
    async.waterfall([
        ...
        function (keyLst) {
            let paymentKey = config.coin + ':payments:all';
            if(urlParts.query.address){
                for(let i = 0;i < keyLst.length; i++){
                    if(keyLst[i].indexOf(urlParts.query.address) > 0){
                        paymentKey = keyLst[i];
                        break;
                    }
                }
...
/**
 * Return PayRecords
 * params:address/ts/te/ps/pe
 * http://127.0.0.1:8117/get_payRecords?address=1dtxf4e89n52eyttgnwhrjwgesez3edk8k9gfcjfq94yas0qb4j67vqz7&ts=1575954965&te=1575966965
 */
function handleGetPayRecords(urlParts, response){
    async.waterfall([
        function (callback) {
            redisClient.keys(config.coin + ':payRecord:*', callback);
        },
        function (keyLst) {
            let paymentKey = '';
            if (urlParts.query.address) {
            ...
            let timeStart = parseInt(urlParts.query.ts || 0); // startTIme
            let timeEnd = parseInt(urlParts.query.te || 0); // endTime

            let pageStart = parseInt(urlParts.query.ps || 0);
            let pageStop  = parseInt(urlParts.query.pe || 0);
            ...
            redisClient.lrange(paymentKey, pageStart, pageStop, function(err, result){
                    let data;
                    let count;
                    if (err) {
                        data = { error: 'Query failed' };
                    ...
                    data.forEach(element => {
                        ...
                        let time = parseInt(arr[4]||0);
                        if (timeStart && timeEnd) {
                            if (time >= timeStart && time <= timeEnd) {
                                if (time !== 0) {
                                    replyS.push(element);
                                    total += parseFloat(arr[0] || 0);
                                }                                
                            }
                        }else{
                            total += parseFloat(arr[0] || 0);
                    ...
                    if (timeStart && timeEnd && replyS.length >= 0){
                        reply = JSON.stringify(replyS);   
                        count = replyS.length;                     
                    }

                    let tmp = '{"lst":' + reply + ',';
                    tmp +='"total":' + utils.toFixed(total, 2) + ',"pstart":' + pageStart + ',"pstop":' + pageStop + ',"count":' + count +'}';
                    reply = tmp;
...
/**
 * params:ty/ts/te/ps/pe
 * @param {ty=hashrate/payment/minerout&ts=time&te=time&ps=start&pe=end} urlParts 
 * @param {*} response 
 */
function handleGetMsg(urlParts, response) {
    let type  = urlParts.query.ty || 0;
    let types = ['hashrate', 'payment', 'minerout'];
    ...
    if (types.includes(type)) {
        redisClient.zrange(config.coin + ':message:' + type, pageStart, pageStop, 'WITHSCORES', function (error, result) {
            let data;
            let count;
            if (error) {
                data = error;
            ...
            let tmp = '{"lst":' + reply + ',';
            tmp += '"pstart":' + pageStart + ',"pstop":' + pageStop + ',"count":' + (count || 0) + '}';
            reply = tmp;
    ...
    async.waterfall([
        ...
        function (workerData, blocks, callback) {
            ...
            for (let i = 0; i < workerData.length; i++) {
                stats.totalOwed += parseFloat(workerData[i][0]) || 0;
                stats.totalPaid += parseFloat(workerData[i][1]) || 0;
                stats.totalWorkers++;
            }
...
function handleAdminLog(urlParts, response) {
    ...
    if (!file.match(/^[\u9fa5A-Za-z0-9-\_]+\.log$/)) {
        response.end('wrong log file');
    }
    response.writeHead(200, {
