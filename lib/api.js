            ...
            redisClient.multi(redisCommands).exec(function (error, replies) {
                ...
                if (parseInt(data.hashrate) < (parseInt(highestHashrate) / 2)){
                    ...
                    redisClient.zadd(config.coin + ':message:hashrate', now, [
                        ...
                        if (err) {
                            log('error', logSystem, 'Failed inserting message %j', [err]);
                        }
                    });
                }
...
function getNetworkData(callback) {
    async.waterfall([
            ...
            for (let i = height; i > height - tmpHeightCount; i--) {                
                apiInterfaces.rpcDaemon('getblockhash', { height: i }, function (error, hash) {
                    ...
                    apiInterfaces.rpcDaemon('getblockdetail', { block: hash.toString() }, function (error, res) {
                        ...
                        redisClient.zadd(config.coin + ':networkJson', i, [
                            res.hash,
                            res.time,
                            res.height,
                            res.bits
                            ...
                            if (err) {
                                log('error', logSystem, 'Failed inserting networkJson %s \n %j', [res.hash, err]);
                            }
                        });
                    });
                });
        ...
        function (data, callback) {
            //从redis中读出数据
            redisClient.zrange(config.coin + ':networkJson', 0, -1, 'WITHSCORES', function (error, results) {
                if (error) {
                    log('error', logSystem, 'Error trying to get networkJson:%j', [error]);
                    callback(true);
                    return;
                }
                
                if (results.length === 0) {
                    //log('info', logSystem, 'No blocks networkJson data in redis');
                    callback(true);
                    return;
                }
                ...
                for (let i = 0; i < results.length; i++) {
                    let nlist = results[i].split(':');
                    if (nlist[3] && nlist[1]) {
                        netHashTotal += (Math.pow(2, Number(nlist[3])));
                        if (i === 0) {
                            nTimeFirst = nlist[1];
                        }
                    }
