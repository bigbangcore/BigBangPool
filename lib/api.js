        ...
        pool: function (callback) {
            redisClient.multi(redisCommands).exec(function (error, replies) {
                 ...
                 * 矿池算力  计算的是连接矿池的所有机器600秒内的hashrate总和除以600
                   全网算力  计算的是当前连接网络的所有机器600秒内的的hashrate总和除以600
                 *  */
...
function getNetworkData(callback) {
    async.waterfall([
        ...
        function (data, callback) {
            ...
            log('info',logSystem,'api_height:%j',[height]);
            log('info',logSystem,'api_tmpHeightCount:%j',[tmpHeightCount]);
            for (let i = height; i > height - tmpHeightCount; i--) {
                ...
                apiInterfaces.rpcDaemon('getblockhash', { height: i }, function (error, hash) {
                    ...
                    apiInterfaces.rpcDaemon('getblockdetail', { block: hash.toString() }, function (error, res) {
                        ...
                        log('info',logSystem,'api_round:%j',[i]);
        ...
        function (data, callback) {
            ...
            redisClient.zrange(config.coin + ':networkJson', 0, -1, 'WITHSCORES', function (error, results) {
                if (error) {
                    log('error', logSystem, 'Error trying to get networkJson data from redis %j', [error]);
                    callback(true);
                    return;
                }
                log('info',logSystem,'api_resA:%j',[results]);
                if (results.length === 0) {
                    log('info', logSystem, 'No blocks networkJson data in redis');
                    callback(true);
                    return;
                }
                log('info',logSystem,'api_resB:%j',[results]);
                ...
                log('info',logSystem,'api_nTotalTime:%j',[nTotalTime]);
                log('info',logSystem,'api_nTimeLast:%j',[nTimeLast]);
                log('info',logSystem,'api_nTimeFirst:%j',[nTimeFirst]);
                log('info',logSystem,'api_netHashrate:%j',[netHashrate]);
