...
function getNetworkData(callback) {
    async.waterfall([
        ...
        function (data, callback) {
            ...
            let tmpHeightCount = height > 30 ? 30 : height;
    ...
    async.waterfall([
        ...
        function (keyLst) {
            ...
            let redisCommands = [
                ...
                ['zrevrange', config.coin + ':payments:' + addressPay, 0, config.api.payments - 1, 'WITHSCORES'],
...
function handleMinerStats(urlParts, response) {
    ...
    if (longpoll) {
        ...
        log('info',logSystem,'---address:%j',[address]);
        ...
        async.waterfall([
            ...
            function (keyLst) {
                ...
                redisClient.multi([
                    ['hgetall', config.coin + ':workers:' + address],
                    //zrevrange Bigbang:payments:
                    ['zrevrange', config.coin + ':payments:' + addressPay, 0, config.api.payments - 1, 'WITHSCORES'],
                    ['keys', config.coin + ':unique_workers:' + address + '~*'],
                    ['get', config.coin + ':charts:hashrate:' + address]
                ]).exec(function (error, replies) {
                    if (error || !replies[0]) {
                        log('info',logSystem,'---replies:%j',[replies]);
                    ...
                    log('info',logSystem,'---workersData:%j',[workersData]);
                    charts.getUserChartsData(address, paymentsData, function (error, chartsData) {
                        let redisCommands = [];
                        for (let i in workersData) {
                        ...
                        log('info',logSystem,'---redisCommands:%j',[redisCommands]);
                        redisClient.multi(redisCommands).exec(function (error, replies) {
                            for (let i in workersData) {
                                let wi = 2 * i;
                                let hi = wi + 1
                            ...
                            log('info',logSystem,'---data:%j',[data]);
...
function handleGetPayments(urlParts, response) {
    ...
    if (urlParts.query.address)
        paymentKey = ':payments:' + urlParts.query.address + "+" + urlParts.query.address;
